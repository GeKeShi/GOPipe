...
class amoebanetd_pipeline():
    def __init__(self, num_classes=10, num_layers=4, num_filters=512):
        super(amoebanetd_pipeline, self).__init__()
        self.num_classes = num_classes
        self.num_layers = num_layers
        self.num_filters = num_filters

        self.channels = num_filters // 4
        self.channels_prev_prev = self.channels
        self.channels_prev = self.channels
        self.reduction_prev = False
        assert num_layers % 3 == 0
        self.repeat_normal_cells = num_layers // 3

    def make_cells(self, reduction, channels_scale, repeat):
        self.channels *= channels_scale

        for i in range(repeat):
            cell = Cell(self.channels_prev_prev,
                        self.channels_prev,
                        self.channels,
                        reduction,
                        self.reduction_prev)

            self.channels_prev_prev = self.channels_prev
            self.channels_prev = self.channels * len(cell.concat)
            self.reduction_prev = reduction

            yield cell

    def reduction_cell(self):
        return next(self.make_cells(reduction=True, channels_scale=2, repeat=1))

    def normal_cells(self):
        return enumerate(self.make_cells(reduction=False, channels_scale=1,
                         repeat=self.repeat_normal_cells))

    def stage0(self):
        layers: NamedModules = OrderedDict()
        layers['stem1'] = Stem(self.channels)
        layers['stem2'] = self.reduction_cell()
        layers['stem3'] = self.reduction_cell()
        return nn.Sequential(layers)

    def stage1(self):
        layers: NamedModules = OrderedDict()
        layers.update((f'cell1_normal{i + 1}', cell) for i, cell in self.normal_cells())
        layers['cell2_reduction'] = self.reduction_cell()
        return nn.Sequential(layers)

    def stage2(self):
        layers: NamedModules = OrderedDict()
        layers.update((f'cell3_normal{i + 1}', cell) for i, cell in self.normal_cells())
        layers['cell4_reduction'] = self.reduction_cell()
        return nn.Sequential(layers)

    def stage3(self):
        layers: NamedModules = OrderedDict()
        layers.update((f'cell5_normal{i + 1}', cell) for i, cell in self.normal_cells())
        # Finally, classifier
        layers['classify'] = Classify(self.channels_prev, self.num_classes)
        return nn.Sequential(layers)
