...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        model = models.__dict__[args.arch](num_classes=1000, pretrained=True)
    else:
    ...
    # train_dataset = imagenet_lmdb_dataset(traindir,
    ...
    #                                           normalize
    #                                       ]))
    # val_dataset = imagenet_lmdb_dataset(valdir,
    #                                     transform=transforms.Compose([
    ...
    # datadir = os.getcwd()
    # train_dataset = datasets.CIFAR10(datadir, train=True, download=False,
    #                                  transform=transforms.Compose([
    #                                      transforms.RandomResizedCrop(224),
    #                                      transforms.RandomHorizontalFlip(),
    #                                      transforms.ToTensor(),
    #                                      normalize,
    #                                  ]))
    # val_dataset = datasets.CIFAR10(datadir, train=False, download=False,
    #                                transform=transforms.Compose([
    #                                    transforms.RandomResizedCrop(224),
    #                                    transforms.RandomHorizontalFlip(),
    #                                    transforms.ToTensor(),
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        optimizer.step()
        if epoch < 5:
            warmup(optimizer, i, epoch, len(train_loader), args)
        ...
        if i % 500 == 99 and args.rank == 0:
            print_loss = format(losses.avg, '.4f')
            print_time = format(time.time(), '.4f')
            print_acc1 = format(top1.avg, '.4f')
            with open('./VGG-results/DP-loss.txt', 'a') as file:
                file.write(str(epoch) + '-' + str(epoch * loader_len + i) +
...
def validate(val_loader, model, criterion, epoch, args):
    ...
    with torch.no_grad():
        ...
        if args.rank == 0:
            ...
            print_time = format(time.time(), '.4f')
            with open('./VGG-results/DP-acc.txt', 'a') as file:
                file.write(str(epoch) + '-' + str(epoch * loader_len + i) +
                           '-' + str(print_time) + '-' + str(print_acc1) +
                           '-' + str(print_acc5) + '\n')
...
def warmup(optimizer, step, epoch, epoch_length, args):
    if args.lr_warmup:
        lr = args.lr * float(1 + step + epoch * epoch_length)/(5.0*epoch_length)
