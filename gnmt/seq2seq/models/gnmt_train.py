...
class Stage0(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        src_length = torch.transpose(src_length.expand
                                     (1, 2048, src_length.shape[0]), 2, 1).float()
        tgt_input = tgt_input.expand(2048, tgt_input.shape[0], tgt_input.shape[1])
        tgt_input = torch.transpose(torch.transpose(tgt_input, 1, 0), 2, 1).float()
...
class Stage3(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        # lstm1
        tgt_embed_drop = self.dropout(tgt_embed)
        out1, hidden = self.lstm1(tgt_embed_drop, None)
...
class Stage4(torch.nn.Module):
    def __init__(self, checkpoint_=True):
    ...
    def attn_cal(self, out1, out8, src_length):
        # out1, out8 = inputs
    ...
    def forward(self, input):
        ...
        if self.checkpoint:
            attn, out1 = checkpoint(self.attn_cal, out1, out8, src_length)
        else:
            attn, out1 = self.attn_cal(out1, out8, src_length)
