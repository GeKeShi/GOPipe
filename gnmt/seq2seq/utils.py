...
class AverageMeter:
    ...
    def reduce(self, op, group):
        ...
        dist.all_reduce(avg, op=_reduce_op.SUM, group=group)
        dist.all_reduce(_sum, op=_reduce_op.SUM, group=group)
        self.avg = avg.item()
        self.sum = _sum.item()
