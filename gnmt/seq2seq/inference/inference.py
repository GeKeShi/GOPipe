...
def gather_predictions(preds, num_worker, group):
    if num_worker > 1:
        all_preds = [preds.new(preds.size(0), preds.size(1)) for i in range(num_worker)]
        dist.all_gather(all_preds, preds, group=group)
...
class Translator:
    ...
    def evaluate(self, epoch, iteration, summary):
        ...
        for i, (src, indices) in enumerate(self.loader):
            ...
            global_batch_size = batch_size * self.num_worker
            ...
            preds = gather_predictions(preds, self.num_worker, self.worker_group).cpu()
        ...
        if self.num_worker>1:
            tot_tok_per_sec.reduce('sum', self.worker_group)
            enc_seq_len.reduce('mean', self.worker_group)
            dec_seq_len.reduce('mean', self.worker_group)
            batch_time.reduce('mean', self.worker_group)
            iterations.reduce('sum', self.worker_group)
