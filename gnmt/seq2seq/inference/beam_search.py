...
# class SequenceGenerator_pipeline:
#     """
#     Generator for the autoregressive inference with beam search decoding.
#     """
#     def __init__(self, model, beam_size=5, max_seq_len=100, cuda=False,
#                  len_norm_factor=0.6, len_norm_const=5, cov_penalty_factor=0.1,
#                  for_s=None, for_r=None, back_s=None, back_r=None):
#         """
#         Constructor for the SequenceGenerator.
#
#         Beam search decoding supports coverage penalty and length
...
#                    value=self.cov_penalty_factor, sync=False)
#
#     def greedy_search(self, batch_size, initial_input, out_up, src_length):
#         """
#         Greedy decoder.
#
#         :param batch_size: decoder batch size
...
#             src_seq_lengths, None]
#
#         :param out_up: same to src_length
#         :param src_length: data from up-rank
#
#         returns: (translation, lengths, counter)
#             translation: (batch_size, max_seq_len) - indices of target tokens
#             lengths: (batch_size) - lengths of generated translations
...
#         translation[:, 0] = BOS
#         words = initial_input
#         context = [out_up, src_length, None]
#
#         if self.batch_first:
#             word_view = (-1, 1)
#             ctx_batch_dim = 0
...
# # XXX
#             words = words.view(word_view)
#             out, attn_stage, h, attn, enc_context, src_length = self.model(context[0], context[1], words, True)
#             out = out.detach()
#             attn_stage = attn_stage.detach()
#             h_0 = h[0].detach()
#             h_1 = h[1].detach()
#             self.for_s.send([out, attn_stage, [h_0, h_1]])
#             output = self.back_r.recv()
#
#             words, logprobs, hidden = output
#             words = words.cuda()
#             logprobs = logprobs.cuda()
#             attn = attn.cuda()
#             enc_context = enc_context.cuda()
#             hidden = hidden.cuda()
#             context = [enc_context, src_length, hidden]
# # XXX
#             translation[active, idx] = words
#             lengths[active] += 1
#
...
#         return translation, lengths, counter
#
#     def beam_search(self, batch_size, initial_input, out_up, src_length):
#         """
#         Beam search decoder.
#
#         :param batch_size: decoder batch size
...
#
# # XXX
#             out, attn_stage, h, attn, enc_context, src_length = self.model(context[0],
#                                                                      context[1], words, True)
#             out = out.detach().clone()
#             attn_stage = attn_stage.detach().clone()
#             enc_context = enc_context.detach()
#             h_0 = h[0].detach().clone()
#             h_1 = h[1].detach().clone()
