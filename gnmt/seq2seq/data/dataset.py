...
class TextDataset(Dataset):
    ...
    def get_loader(self, batch_size=1, seeds=None, shuffle=False,
                   ...
                   batching=None, batching_opt={}, world_size=None, rank=None):
        ...
        if shuffle:
            if batching == 'random':
                sampler = DistributedSampler(self, batch_size, seeds, world_size, rank)
            elif batching == 'sharding':
                sampler = ShardingSampler(self, batch_size, seeds,
                                          batching_opt['shard_size'], world_size, rank)
            elif batching == 'bucketing':
                sampler = BucketingSampler(self, batch_size, seeds,
                                           batching_opt['num_buckets'], world_size, rank)
            else:
                raise NotImplementedError
        else:
            sampler = StaticDistributedSampler(self, batch_size, pad, world_size, rank)
